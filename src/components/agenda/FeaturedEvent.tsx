...
export default function FeaturedEvent({ event }: { event: Agenda }) {
  ...
  return (
    <div
      className="group relative h-full text-white rounded-[8px_8px_76px_8px] overflow-hidden border border-[#4d4d4d] hover:border-white hover:scale-102 transition-all duration-200 cursor-pointer"
      onClick={() => {
        if (event.link) {
